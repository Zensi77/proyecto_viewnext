...
public interface JpaProductCartRepository extends JpaRepository<ProductCartEntity, ProductCartEntity.ProductCartPK> {
    ...
    @Modifying
    @Query("DELETE FROM ProductCartEntity pc WHERE pc.id.productId = :productId AND pc.id.cartId = :cartId")
    void deleteByProductAndCart(UUID productId, UUID cartId); // Cambia el nombre del método para que sea más claro

    @Modifying
    @Query("DELETE FROM ProductCartEntity pc WHERE pc.id.productId = :productId")
